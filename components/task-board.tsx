...
function TaskCard({
  ...
  return (
    <motion.div
      layout
      variants={item}
      className="group relative bg-white dark:bg-zinc-800 rounded-xl border border-gray-100 dark:border-zinc-700 p-4 shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer"
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="relative">
          <button
            ...
            className="opacity-0 group-hover:opacity-100 p-1 rounded-md text-gray-400 hover:bg-gray-100 dark:hover:bg-zinc-700 transition-all"
          >
...
export function TaskBoard({
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Toolbar */}
      <div className="flex-shrink-0 flex items-center justify-between px-8 py-5 border-b border-gray-100 dark:border-zinc-800">
        <div className="flex items-center gap-3">
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">Tasks</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onNewTask}
            className="flex items-center gap-1.5 px-3 py-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-sm font-semibold rounded-lg hover:opacity-90 transition-all"
          >
      ...
      {isEmpty ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center py-24">
          <div className="w-16 h-16 rounded-2xl bg-gray-100 dark:bg-zinc-800 flex items-center justify-center">
            <CheckCircle2 size={28} className="text-gray-300 dark:text-zinc-600" />
          </div>
          <div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          >
            {/* Header */}
            <div className="grid grid-cols-[2fr_1fr_1fr_1fr_auto] gap-4 px-4 py-2.5 bg-gray-50 dark:bg-zinc-800 text-xs font-semibold text-gray-400 dark:text-zinc-500 uppercase tracking-wider">
              <span>Title</span>
              <span>Priority</span>
              <span>Status</span>
              <span>Due</span>
            ...
            {filtered.map((task) => (
              <motion.div
                key={task.id}
                variants={item}
                className="group grid grid-cols-[2fr_1fr_1fr_1fr_auto] gap-4 px-4 py-3 items-center hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors"
              >
